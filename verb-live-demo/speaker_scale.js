...
server = "wss://s12345.9j2fjactu.cluster.verb.tech:443";
var janus = null;
var publisher = null, mixer = null, streaming = null;
var opaqueId = "verb-speaker-"+Janus.randomString(12);

var myroom = 12345;	// Demo room: we'll use the same ID for all resources
var myusername = null;
var myid = null;
	...
	janus.attach(
		{
			...
			success: function(pluginHandle) {
				...
				// 							host: 'stream-qa.verb.tech',			// We know the address of Janus already
	...
	janus.attach(
		{
			...
			success: function(pluginHandle) {
				...
				// 						host: "192.168.16.2",				// IP address of the mountpoint
				// 						host_family: "ipv4",			// Let's just assume it's an IPv4 address
				// 						audio_port: 9001,		// Audio port of the mountpoint
				// 						audio_rtcp_port: 9002,
	...
	janus.attach(
		{
			...
			webrtcState: function(on) {
				...
				// 		host: "192.168.16.2",					// IP address of the mountpoint
				// 		host_family: "ipv4",				// Let's just assume it's an IPv4 address
				// 		video_port: 9101,		// Video port of the mountpoint
				// 		video_rtcp_port: 9102	// Video RTCP port of the mountpoint (needed for keyframes)
				// 	}, success: function(result) {
				// 		// Let's make sure everything went fine
				// 		if(result.error) {
			...
			onmessage: function(msg, jsep) {
				...
				if(event) {
					if(event === "joined") {
						// Successfully joined, create the audio WebRTC PeerConnection now
						myid = msg["id"];
						bootbox.alert("Your speaker ID: " + myid);
